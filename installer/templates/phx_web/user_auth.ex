defmodule <%= @web_namespace %>.UserAuth do
  use <%= @web_namespace %>, :verified_routes

  import Plug.Conn
  import Phoenix.Controller

  alias <%= @app_module %>.Core.Users

  # Make the remember me cookie valid for 60 days.
  # If you want bump or reduce this value, also change
  # the token expiry itself in UserToken.
  @max_age 60 * 60 * 24 * 60
  @remember_me_cookie "_<%= @app_name %>_web_user_remember_me"
  ...
  def log_out_user(conn) do
    ...
    if live_socket_id = get_session(conn, :live_socket_id) do
      <%= @web_namespace %>.Endpoint.broadcast(live_socket_id, "disconnect", %{})
    end
  ...
  the current_user:
      ...
      defmodule <%= @web_namespace %>.PageLive do
        ...
        on_mount {<%= @web_namespace %>.UserAuth, :mount_current_user}
        ...
      end
