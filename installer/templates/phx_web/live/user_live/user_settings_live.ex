defmodule <%= @web_namespace %>.UserSettingsLive do
  ...
  def render(assigns) do
    ~H"""
    <p class="text-center">
      Account Settings <span>Manage your account email address and password settings</span>
    </p>
    ...
    <div class="space-y-12 divide-y">
      <div>
        ...
        >
          ...
          />
          <:actions>
            <.button phx-disable-with="Changing..." label="Change Email" />
          </:actions>
        </.simple_form>
      </div>
      <div>
        <.simple_form
          for={@password_form}
          id="password_form"
          action={UrlHelper.login_path("_action=password_updated")}
          method="post"
        ...
        >
          ...
          />
          <:actions>
            <.button phx-disable-with="Changing..." label="Change Password" />
          </:actions>
        </.simple_form>
      </div>
    </div>
  ...
  def mount(%{"token" => token}, _session, socket) do
    ...
    {:ok, push_navigate(socket, to: UrlHelper.user_settings_path())}
  end
    ...
    case Users.apply_user_email(user, password, user_params) do
      {:ok, applied_user} ->
        Users.deliver_user_update_email_instructions(
          applied_user,
          user.email,
          &unverified_url(socket, UrlHelper.user_confirm_email_path(&1))
        )
