...
defmodule Mix.Tasks.Flowy.New.UmbrellaTest do
  ...
  test "new with umbrella and defaults" do
    in_tmp("new with umbrella and defaults", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella"])
      ...
      assert_file(web_path(@app, "lib/phx_umb_web/router.ex"), fn file ->
        ...
        assert file =~ ~s[live(\"/\", Live.HomeLive, :show)]
      end)
  ...
  test "new with --no-dashboard" do
    in_tmp("new with no_dashboard", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella", "--no-dashboard"])
  ...
  test "new with --no-dashboard and --no-live" do
    in_tmp("new with no_dashboard and no_live", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella", "--no-dashboard", "--no-live"])
  ...
  test "new with --no-html" do
    in_tmp("new with no_html", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella", "--no-html"])
  ...
  test "new with --no-assets" do
    in_tmp("new with no_assets", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella", "--no-assets"])
  ...
  test "new with binary_id" do
    in_tmp("new with binary_id", fn ->
      Mix.Tasks.Flowy.New.run([@app, "--umbrella", "--binary-id"])
  ...
  test "new with uppercase" do
    in_tmp("new with uppercase", fn ->
      Mix.Tasks.Flowy.New.run(["phxUmb", "--umbrella"])
  ...
  test "new with path, app and module" do
    in_tmp("new with path, app and module", fn ->
      ...
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--app", @app, "--module", "PhoteuxBlog"])
  ...
  test "new defaults to pg adapter" do
    in_tmp("new defaults to pg adapter", fn ->
      ...
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella"])
  ...
  test "new with mysql adapter" do
    in_tmp("new with mysql adapter", fn ->
      app = "custom_path"
      project_path = Path.join(File.cwd!(), app)
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--database", "mysql"])
  ...
  test "new with sqlite3 adapter" do
    in_tmp("new with sqlite3 adapter", fn ->
      app = "custom_path"
      project_path = Path.join(File.cwd!(), app)
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--database", "sqlite3"])
  ...
  test "new with mssql adapter" do
    in_tmp("new with mssql adapter", fn ->
      app = "custom_path"
      project_path = Path.join(File.cwd!(), app)
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--database", "mssql"])
  ...
  test "new with invalid database adapter" do
    in_tmp("new with invalid database adapter", fn ->
      ...
      assert_raise Mix.Error, ~s(Unknown database "invalid"), fn ->
        Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--database", "invalid"])
      end
    end)
  end
  ...
  test "new with bandit web adapter" do
    in_tmp("new with bandit web adapter", fn ->
      app = "custom_path"
      project_path = Path.join(File.cwd!(), app)
      Mix.Tasks.Flowy.New.run([project_path, "--umbrella", "--adapter", "bandit"])
  ...
  test "new with invalid args" do
    assert_raise Mix.Error, ~r"Application name must start with a letter and ", fn ->
      Mix.Tasks.Flowy.New.run(["007invalid", "--umbrella"])
    end

    assert_raise Mix.Error, ~r"Application name must start with a letter and ", fn ->
      Mix.Tasks.Flowy.New.run(["valid1", "--app", "007invalid", "--umbrella"])
    end

    assert_raise Mix.Error, ~r"Module name must be a valid Elixir alias", fn ->
      Mix.Tasks.Flowy.New.run(["valid2", "--module", "not.valid", "--umbrella"])
    end

    assert_raise Mix.Error, ~r"Module name \w+ is already taken", fn ->
      Mix.Tasks.Flowy.New.run(["string", "--umbrella"])
    end

    assert_raise Mix.Error, ~r"Module name \w+ is already taken", fn ->
      Mix.Tasks.Flowy.New.run(["valid3", "--app", "mix", "--umbrella"])
    end

    assert_raise Mix.Error, ~r"Module name \w+ is already taken", fn ->
      Mix.Tasks.Flowy.New.run(["valid4", "--module", "String", "--umbrella"])
    end
  end

  test "invalid options" do
    assert_raise OptionParser.ParseError, fn ->
      Mix.Tasks.Flowy.New.run(["valid5", "-database", "mysql", "--umbrella"])
    end
  end
        ...
        for dir <- [cwd, umbrella_path] do
          File.cd!(dir, fn ->
            assert_raise Mix.Error,
                         ...
                         fn ->
                           Mix.Tasks.Flowy.New.Ecto.run(["valid"])
                         end
          end)
        end
      end)
  ...
  describe "web task" do
        ...
        for dir <- [cwd, umbrella_path] do
          File.cd!(dir, fn ->
            assert_raise Mix.Error,
                         ...
                         fn ->
                           Mix.Tasks.Flowy.New.Web.run(["valid"])
                         end
          end)
        end
      end)
    ...
    test "generates web-only files", %{tmp_dir: tmp_dir} do
      in_tmp(tmp_dir, fn ->
        ...
        decline_prompt()
        Mix.Tasks.Flowy.New.Web.run(["another"])
