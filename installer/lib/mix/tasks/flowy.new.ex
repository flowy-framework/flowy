defmodule Mix.Tasks.Flowy.New do
  @moduledoc """
  Creates a new Flowy project.

  It expects the path of the project as an argument.

      $ mix flowy.new PATH [--module MODULE] [--app APP]
  ...
  ## Options
    ...
    * `--no-dashboard` - do not include Flowy.LiveDashboard
    ...
    * `-v`, `--version` - prints the Flowy installer version

  When passing the `--no-ecto` flag, Flowy generators such as
  `flowy.gen.html`, `flowy.gen.json`, `flowy.gen.live`, and `flowy.gen.context`
  ...
  `flowy.gen.html` will no longer work, as important HTML components
  will be missing.

  ## Installation

  `mix flowy.new` by default prompts you to fetch and install your
  dependencies. You can enable this behaviour by passing the
  `--install` flag or disable it with the `--no-install` flag.

  ## Examples

      $ mix flowy.new hello_world

  Is equivalent to:

      $ mix flowy.new hello_world --module HelloWorld

  Or without the HTML and JS bits (useful for APIs):

      $ mix flowy.new ~/Workspace/hello_world --no-html --no-assets

  As an umbrella:

      $ mix flowy.new hello --umbrella
  ...
  """
  use Mix.Task
  alias Flowy.New.{Generator, Project, Single, Umbrella, Web, Ecto}

  @version Mix.Project.config()[:version]
  @shortdoc "Creates a new Flowy v#{@version} application"
  ...
  def run([version]) when version in ~w(-v --version) do
    Mix.shell().info("Flowy installer v#{@version}")
  end
  ...
  def run(argv, generator, path) do
    ...
    case OptionParser.parse!(argv, strict: @switches) do
      {_opts, []} -> Mix.Tasks.Help.run(["flowy.new"])
  ...
  defp prompt_to_install_deps(%Project{} = project, generator, path_key) do
    ...
    maybe_cd(path, fn ->
      ...
      if path_key == :web_path do
        ...
        The PubSub server is typically defined in a `mix flowy.new.ecto` app.
        If you don't plan to define an Ecto app, you must explicitly start
        the PubSub in your supervision tree as:

            {Flowy.PubSub, name: #{inspect(project.app_mod)}.PubSub}
        """)
      end
  ...
  defp elixir_version_check! do
    unless Version.match?(System.version(), "~> 1.14") do
      Mix.raise(
        "Flowy v#{@version} requires at least Elixir v1.14\n " <>
