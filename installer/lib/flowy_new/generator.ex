defmodule Flowy.New.Generator do
  @moduledoc false
  ...
  def inject_umbrella_config_defaults(project) do
    unless File.exists?(Project.join_path(project, :project, "config/dev.exs")) do
      ...
      extra =
        Flowy.New.Umbrella.render(:new, "phx_umbrella/config/extra_config.exs", project.binding)
  ...
  defp phoenix_path(%Project{} = project, true = _dev, umbrella_root?) do
    ...
    if absolute == relative do
      Mix.raise("--dev projects must be generated inside Flowy directory")
    end
