defmodule Mix.Flowy.Schema do
  ...
  def new(schema_name, schema_plural, cli_attrs, opts) do
    ...
    module = Module.concat([base, "Schemas", schema_name])
    repo = opts[:repo] || Module.concat([base, "Repo"])
    file = Mix.Flowy.lib_path(:schemas, ctx_app, basename <> ".ex")
    test_file = Mix.Flowy.test_path(:schemas, ctx_app, basename <> "_test.exs")
  ...
  * In case it is a boolean, we keep it as false
  ...
  def required_keys(schema) do
    schema
    |> required_fields()
    |> Enum.map(fn {key, _} -> key end)
  end

  def all_fields(%{attrs: attrs}) do
    attrs
    |> Enum.map(fn {key, _} -> key end)
