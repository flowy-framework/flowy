defmodule Mix.Tasks.Flowy.Gen.Live do
  ...
  Generates LiveView, templates, and core for a resource.
      ...
      mix flowy.gen.live User users name:string age:integer

  The first argument is the schema module.  The schema is responsible for
  ...
  types.  See `mix help flowy.gen.schema` for more information on attributes.

  When this command is run LiveViews and
  components, including `UserLive.Index`, `UserLive.Show`, and
  `UserLive.FormComponent` modules for the new resource will be created.

  Overall, this generator will add the following files:

    * a core module in `lib/core/users.ex` for the accounts API
    * a schema in `lib/schemas/user.ex`, with a `users` table
  ...
  A migration file for the repository and test files for the core and
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix flowy.gen.live User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix flowy.gen.live User users --web Sales
  ...
  for file generation control.
      ...
      mix flowy.gen.live User users --no-schema

  In the cases above, tests are still generated, but they will all fail.

  You can also change the table name or configure the migrations to
  use binary ids for primary keys, see `mix help flowy.gen.schema` for more
  information.
  """
  use Mix.Task
