defmodule Mix.Tasks.Flowy.Gen.Html do
  ...
  Generates controller, HTML views, and context for an HTML resource.
      ...
      mix flowy.gen.html Accounts User users name:string age:integer
  ...
  Alternatively, the `--context-app` option may be supplied to the generator:
      ...
      mix flowy.gen.html Sales User users --context-app warehouse
  ...
  module name, for example:
      ...
      mix flowy.gen.html Sales User users --web Sales
  ...
  def run(args) do
    if Mix.Project.umbrella?() do
      Mix.raise(
        "mix flowy.gen.html must be invoked from within your *_web application root directory"
      )
    end
  ...
  def copy_new_files(%Context{} = context, paths, binding) do
    ...
    Mix.Phoenix.copy_from(paths, "priv/templates/flowy.gen.html", binding, files)
  ...
  def inputs(%Schema{} = schema) do
    ...
    |> Enum.map(fn
      {key, :integer} ->
        ~s(<.text field={f[#{inspect(key)}]} type="number" label="#{label(key)}" />)

      {key, :float} ->
        ~s(<.text field={f[#{inspect(key)}]} type="number" label="#{label(key)}" step="any" />)

      {key, :decimal} ->
        ~s(<.text field={f[#{inspect(key)}]} type="number" label="#{label(key)}" step="any" />)

      {key, :boolean} ->
        ~s(<.text field={f[#{inspect(key)}]} type="checkbox" label="#{label(key)}" />)

      {key, :text} ->
        ~s(<.text field={f[#{inspect(key)}]} type="text" label="#{label(key)}" />)

      {key, :date} ->
        ~s(<.text field={f[#{inspect(key)}]} type="date" label="#{label(key)}" />)

      {key, :time} ->
        ~s(<.text field={f[#{inspect(key)}]} type="time" label="#{label(key)}" />)

      {key, :utc_datetime} ->
        ~s(<.text field={f[#{inspect(key)}]} type="datetime-local" label="#{label(key)}" />)
  ...
  def card_fields(%Schema{} = schema) do
    schema.attrs
    |> Enum.reject(fn {_key, type} -> type == :map end)
    |> Enum.map(fn
      {key, :integer} ->
