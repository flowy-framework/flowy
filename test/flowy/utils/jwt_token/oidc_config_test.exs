defmodule Flowy.Utils.JwtToken.OIDCConfigTest do
  use ExUnit.Case, async: true
  alias Flowy.Utils.JwtToken.OIDCConfig

  describe "OIDCConfig" do
    @describetag :oidc_config
    test "pem/0 returns the correct value" do
      assert OIDCConfig.pem() ==
               "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
    end

    test "public_keys/0 returns the correct value" do
      assert OIDCConfig.public_keys() == %{
               "keys" => [
                 %{
                   "alg" => "RS256",
                   "e" => "AQAB",
                   "kid" => "iTMxG6ONYMk-jsWFa8MwwG_BvTeNQRaRqTpcPBt8J3g",
                   "kty" => "RSA",
                   "n" =>
                     "q_eqqAnGN1EN6hj1u7RTET738dKcG9s8id6lyEHXG3sNMtnc0EV3D6AaDmG-qKPF6pyRD2EWOYrb5hJCKhJZCiYW8TBwEQim8-ZJVYhFCKBlnT-JtLLXSA32_Xo_ihIJIvY21lzPw0Z53T8zysFmEIsaOEfcpCoBOaqBaLBGXMp_YraV4bpFGIXRsZ1f394gR_6LFQKcJpOPQhBSGCgXpv15LNgXRE13UUlgOPQVws1A5XLtpUojApKXEiFFL9WycddLsKoi8CVSD-ZxUYC6f5utRHM6-AxKnqpA0eSMfTtKT3N7n57M5WlbAB9O-ahuVkGDzCLMsBt5q3Ml0Oy-lQ",
                   "use" => "sig"
                 }
               ]
             }
    end

    test "iss/0 returns the correct value" do
      assert OIDCConfig.iss() == "https://local.dev/"
    end

    test "aud/0 returns the correct value" do
      assert OIDCConfig.aud() == "https://flowy.local.dev"
    end

    test "jwks_uri/0 returns the correct value" do
      assert OIDCConfig.jwks_uri() == "http://localhost/jwks"
    end

    test "claims/1 returns the correct value" do
      assert OIDCConfig.claims() == [
               iss: "https://local.dev/",
               aud: ["https://flowy.local.dev"],
               skip: []
             ]
    end
  end
end
