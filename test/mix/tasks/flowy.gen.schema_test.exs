...
defmodule Mix.Tasks.Flowy.Gen.SchemaTest do
  ...
  test "build" do
    in_tmp_project("build", fn ->
      ...
      assert %Schema{
               alias: Post,
               module: Flowy.Schemas.Blog.Post,
               repo: Flowy.Repo,
               migration?: true,
      ...
      assert String.ends_with?(schema.file, "lib/flowy/schemas/blog/post.ex")
    end)
  end
  ...
  test "generates schema", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file("lib/flowy/schemas/blog/post.ex")
      assert_file("test/flowy/schemas/blog/post_test.exs")
  ...
  test "allows a custom migration dir", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file(migration, fn file ->
        assert file =~ "defmodule Flowy.Repo.Migrations.CreateBlogPosts do"
      end)
    end)
  end
  ...
  test "generates nested schema", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file(migration, fn file ->
        assert file =~ "defmodule Flowy.Repo.Migrations.CreateUsers do"
        assert file =~ "create table(:users) do"
      end)

      assert_file("lib/flowy/schemas/blog/admin/user.ex", fn file ->
        assert file =~ "defmodule Flowy.Schemas.Blog.Admin.User do"
  ...
  test "generates unique indices", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file(migration, fn file ->
        assert file =~ "defmodule Flowy.Repo.Migrations.CreatePosts do"
      ...
      assert_file("lib/flowy/schemas/blog/post.ex", fn file ->
        assert file =~ "defmodule Flowy.Schemas.Blog.Post do"
  ...
  test "generates schema with proper datetime types", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file("lib/flowy/schemas/blog/comment.ex", fn file ->
  ...
  test "generates schema without extra line break", config do
    in_tmp_project(config.test, fn ->
      ...
      assert_file("lib/flowy/schemas/blog/post.ex", fn file ->
        assert file =~
                 "defmodule Flowy.Schemas.Blog.Post do\n  use Ecto.Schema\n  import Ecto.Changeset"
      end)
    end)
  end
  ...
  describe "inside umbrella" do
    ...
    test "with context_app set to nil", config do
      in_tmp_umbrella_project(config.test, fn ->
        Application.put_env(:flowy, :generators, context_app: nil)
    ...
    test "with context_app", config do
      in_tmp_umbrella_project(config.test, fn ->
        ...
        assert_file("another_app/lib/another_app/schemas/blog/post.ex")
